...
{-# LANGUAGE LambdaCase                 #-}
module Haskit.CodeGen(compile) where
...
data CodeGenState =
    CodeGenState { symbols      :: HashMap String Var
                 , knums        :: DList BC.KNum
                 , kgcs         :: DList BC.KGC
...
data Var = Local LocalIdx | Const ConstIdx | Global ConstIdx
    deriving (Show)
...
expr (P.Constant (P.IntConst v)) =
    fmap Const . knum . BC.KI64 $ fromIntegral v
expr (P.Constant (P.BoolConst v)) =
    fmap Const . knum . BC.KI64 $ if v then 1 else 0
...
compile defs =
    BC.ByteCode $ fmap make defs ++ [frame 0 3 $ execState (runCodeGen $ setup defs) (initial [])]
    where
      make def =
        frame (fromIntegral . length $ P.definitionParams def) 0 .
            execState (runCodeGen . definition $ P.definitionBody def) . initial $ P.definitionParams def
      globals =
        let names = fmap P.definitionName defs
        in fmap (second Global) $ zip names [0..]
      initial params =
        CodeGenState { symbols = Map.fromList $ globals ++ zip params (fmap Local [0..])
                     , knums = L.empty
                     , kgcs = L.empty
